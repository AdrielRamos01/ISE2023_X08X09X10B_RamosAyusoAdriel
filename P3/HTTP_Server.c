...
///* Thread declarations */
  ...
  static void Display  (void *arg);
	...
	static void LowPowerB     (void *arg);
...
static __NO_RETURN void sleep (void *arg) {
//Retardo de 15 segundos
osDelay(15000);
//Desactivamos el modo de barrido de los leds y los leds
LEDrun = false;
...
HAL_SuspendTick(); /* Request to enter SLEEP mode */
HAL_PWR_EnterSLEEPMode(PWR_LOWPOWERREGULATOR_ON, PWR_SLEEPENTRY_WFI); /* Resume Tick interrupt if disabled prior to sleep mode entry */
 HAL_ResumeTick();
 /* Enable ETH CLK */
...
///*----------------------------------------------------------------------------
//  Thread 'LowPower Blinking'
// *---------------------------------------------------------------------------*/
static __NO_RETURN void LowPowerB (void *arg) {
  
  while(1) {
		
		if (0x333 == osThreadFlagsWait(0x333,osFlagsWaitAny, 10U)){
		Init_Timer_LowPower();
		}
...
__NO_RETURN void app_main (void *arg) {
  ...
  TID_SNTP    = osThreadNew (SNTP,         NULL, NULL);
	TID_SLEEP   = osThreadNew (sleep,        NULL, NULL);
	TID_LPB     = osThreadNew (LowPowerB,    NULL, NULL);
